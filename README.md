...
Контроллеры SpeedyBee (F405 V4, и др.) имеют встроенный BLE-модуль (ESP32), через который приложение SpeedyBee на телефоне общается с FC по протоколу MSP. Однако полноценные GCS — **Mission Planner** и **QGroundControl** — не умеют подключаться к FC по BLE напрямую.
